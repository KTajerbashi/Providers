...
namespace SoapParserWCF.ProxyFactory
{
    public class SchemaInjection
    {
        ...
        public SchemaInjection(ServiceDescriptionImporter serviceImport, ServiceDescription ServiceDesc,
        ...
        private void InjectInnerNameSpaces(ServiceDescription ServiceDesc, Uri InnerUri)
        {
            ...
            foreach (Import schemaImport in ServiceDesc.Imports)
            {
                ...
                if (this.Username.IsNotEmpty())
                    webc.Credentials = new NetworkCredential(this.Username, this.Password);

                using (Stream schemaStream = webc.OpenRead(schemaUri))
                {
                    try
                    {
